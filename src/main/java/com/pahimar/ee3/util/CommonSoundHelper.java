...
public class CommonSoundHelper {
    ...
    public static void playSoundAtPlayer(EntityPlayer entityPlayer, String soundName, float volume, float pitch, double range) {
        Network.INSTANCE.sendToAllAround(new MessageSoundEvent(entityPlayer, soundName, volume, pitch), new NetworkRegistry.TargetPoint(entityPlayer.worldObj.provider.getDimension(), entityPlayer.posX, entityPlayer.posY, entityPlayer.posZ, range));
    }
}
