...
2. Download the latest version of MCP from [here] (http://mcp.ocean-labs.de/index.php/MCP_Releases) , e.g. mcp722.zip. Install MCP dependencies as listed on the website if neccessary.
...
8. Download the latest forge source for Minecraft 1.4.5(pre) and unzip it into `mcp` so that `mcp/forge/install.sh` exists. You need at least Forge 6.4.0.xxx, best way is to get it from [here] (http://files.minecraftforge.net/): http://files.minecraftforge.net/
